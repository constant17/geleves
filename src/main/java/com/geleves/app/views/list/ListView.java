...
public class ListView extends VerticalLayout {
    ...
    private void configureGrid() {
        ...
        grid.setSizeFull();
        grid.setColumns("nom", "prenom", "email");
        grid.addColumn(contact -> contact.getStatus().getName()).setHeader("Statut");
        grid.addColumn(contact -> contact.getCompany().getName()).setHeader("Parents");
        grid.getColumns().forEach(col -> col.setAutoWidth(true));
    }

    private HorizontalLayout getToolbar() {
        filterText.setPlaceholder("Recherche par nom");
        filterText.setClearButtonVisible(true);
        filterText.setValueChangeMode(ValueChangeMode.LAZY);
        filterText.addValueChangeListener(e -> updateList());

        Button addContactButton = new Button("Ajouter ");
